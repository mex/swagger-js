...
function schemaToHTML(name, schema, models, modelPropertyMacro) {
  ...
  function sanitizeReferenceName(name) {
    // Remove potential resource/service prefix
    var ref = name.replace(/^(.+?)json__(.+?)$/i, '$2');
    // Remove suffixed request/response text
    ref = ref.replace(/Request$/i, '').replace(/Response$/i, '');
    // Remove suffixed create/update text
    ref = ref.replace(/Create$/i, '').replace(/Update$/i, '');
    return ref;
  }
  ...
  function primitiveToOptionsHTML(schema, html) {
    ...
    if (!_.isUndefined(schema.format)) {
      options += optionHtml('Format', '<code>' + schema.format + '</code>');
    }
    ...
    case 'string':
      if (schema.minLength && schema.maxLength && schema.minLength === schema.maxLength) {
        options += optionHtml('Length', schema.minLength);
      } else {
        if (schema.minLength) {
    ...
    case 'number':
      ...
      if (schema.exclusiveMinimum) {
        options += optionHtml('Exclusive min.', 'yes');
      }
      ...
      if (schema.exclusiveMaximum) {
        options += optionHtml('Exclusive max.', 'yes');
      }
    ...
    if (isArray) {
      ...
      if (schema.uniqueItems) {
        options += optionHtml('Unique items', 'yes');
      }
  ...
  function processModel(schema, name) {
    ...
    if (name.indexOf('Inline Model') === -1) {
      if (!_.isUndefined(schema.title)) {
        html = strongOpen + sanitizeReferenceName(schema.title) +  strongClose;
      } else {
        html = strongOpen + sanitizeReferenceName(name) +  strongClose;
      }
    }
